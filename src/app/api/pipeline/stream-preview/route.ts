  ...
  try {
    ...
    if (cropBox && width && height) {
      if (cropBox.width <= 1.0 && cropBox.height <= 1.0) {
        ...
        console.log(`[Stream Preview API] Visier-Skalierung: Referenz ${cropBox.imageWidth}x${cropBox.imageHeight} -> Bild ${width}x${height} (sx=${sx.toFixed(4)}, sy=${sy.toFixed(4)})`);
        cropBox = {
        ...
        console.warn(`[Stream Preview API] cropBox überschreitet Bildgrenzen: Skaliere von vermuteter Auflösung ${assumedOrigW}x${assumedOrigH} auf ${width}x${height}`);
        cropBox = {
          x: Math.round(cropBox.x * sx),
          y: Math.round(cropBox.y * sy),
          width: Math.round(cropBox.width * sx),
